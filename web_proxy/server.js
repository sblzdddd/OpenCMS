...
["application/json", "application/x-www-form-urlencoded", "text/plain", "text/html"].forEach(type =>
...
// Utility functions
const rewriteCookies = cookies =>
  cookies &&
  (Array.isArray(cookies) ? cookies : [cookies]).map(cookie => {
    let updated = cookie.replace(/domain=([^;]+)/gi, `domain=${PROXY_HOST}`).replace(/;\s*secure/gi, "");
...
fastify.register(async function (fastify) {
  ...
  fastify.get("/redirect-location/:domain/*", async (req, reply) => {
    ...
    const fullUrl =
      req.query && Object.keys(req.query).length ? `${targetUrl}?${new URLSearchParams(req.query)}` : targetUrl;
  ...
  fastify.all("/proxy/:domain/*", async (req, reply) => {
    ...
    try {
      ...
      if (!["GET", "HEAD"].includes(req.method) && req.body !== undefined) {
        requestBody = typeof req.body === "object" && !Buffer.isBuffer(req.body) ? JSON.stringify(req.body) : req.body;
        ...
        if (lowerKey === "set-cookie") {
          ...
          const mapAbsoluteToProxy = loc => {
            ...
            Object.entries(TARGET_DOMAINS).forEach(([targetDomain, targetUrl]) => {
              out = out.replace(
                new RegExp(targetUrl.replace("https://", "https?://"), "g"),
      ...
      const isTextContent = ["text/html", "application/javascript", "text/css", "application/json", "text/plain"].some(
        ...
        try {
          ...
          if (!bodyText.includes("\0") && !bodyText.includes("\uFFFD")) {
            Object.entries(TARGET_DOMAINS).forEach(([targetDomain, targetUrl]) => {
              ...
              bodyText = bodyText
                .replace(
                  new RegExp(escapedUrl.replace("https\\:\\/\\/", "https?:\\/\\/"), "g"),
                  `${PROXY_BASE_URL}/proxy/${targetDomain}`,
                )
                .replace(
                ...
                .replace(
                  new RegExp(`(href|src|action)=["']\\/((?!\\/)(?!proxy)[^"']*?)["']`, "g"),
